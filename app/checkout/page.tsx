  ...
  if (done) {
    return (
      <div className="flex min-h-[100dvh] w-full flex-col items-center justify-center bg-makola-cream px-4 pb-24 dark:bg-zinc-950">
        ...
        <Link href="/shop" className="btn-outline mt-3">
          Back to Shop
        </Link>
        <BottomNav />
      </div>
    ...
    <div className="min-h-[100dvh] w-full bg-makola-cream pb-28 dark:bg-zinc-950">
      <header className="w-full bg-makola-orange px-4 py-4 text-white">
        <div className="flex w-full items-center justify-between gap-2">
          <Link href="/shop" className="text-sm font-bold sm:text-base">
            ← Cart
          </Link>
      ...
      <div className="mx-auto w-full max-w-3xl space-y-4 px-4 py-4 lg:max-w-5xl">
        {!items.length ? (
          <div className="rounded-2xl bg-white p-8 text-center shadow-sm dark:bg-zinc-900">
            ...
            <Link href="/shop" className="btn-orange mt-4 inline-flex">
              Shop Spices
            </Link>
          </div>
        ) : (
          <>
            <div className="space-y-2 rounded-2xl bg-white p-4 shadow-sm dark:bg-zinc-900">
              <h2 className="text-lg font-bold text-makola-green dark:text-green-400">
                Your spices
              </h2>
              {items.map((i) => (
                <div
                  key={i.spice.id}
                  className="flex items-center justify-between border-b border-orange-50 py-2 last:border-0 dark:border-zinc-800"
                >
                  <div className="min-w-0 flex-1">
                    <p className="truncate font-semibold">
                      {i.spice.emoji} {i.spice.name}
                    </p>
                    <p className="text-sm text-gray-500">
                      {formatGhs(i.spice.pricePerOlonka)} × {i.qty}
                    </p>
                  </div>
                  <div className="flex shrink-0 items-center gap-2">
                    <button
                      type="button"
                      className="h-8 w-8 rounded-full border-2 border-makola-orange font-bold text-makola-orange"
                      onClick={() => setQty(i.spice.id, i.qty - 1)}
                    >
                      −
                    </button>
                    ...
                    <button
                      type="button"
                      className="h-8 w-8 rounded-full bg-makola-orange font-bold text-white"
                      onClick={() => setQty(i.spice.id, i.qty + 1)}
                    >
                      +
                    </button>
            ...
            <div className="rounded-2xl bg-white p-4 shadow-sm dark:bg-zinc-900">
              <h2 className="mb-2 text-lg font-bold text-makola-green dark:text-green-400">
                Stall / Lane
              </h2>
              <input
                className="input-lg"
                placeholder="e.g. Lane 3, near Agbogbloshie"
                value={stallNote}
            ...
            <div className="rounded-2xl bg-white p-4 shadow-sm dark:bg-zinc-900">
              ...
              <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                <button
                  ...
                  className={cn(
                    "rounded-xl border-2 py-3 text-sm font-bold sm:text-base",
                    inside
                      ? "border-makola-green bg-green-50 text-makola-green dark:bg-green-950"
                      : "border-gray-200 dark:border-zinc-700"
                  )}
                >
                  Inside Makola (GHS 10)
                </button>
                <button
                  type="button"
            ...
            <div className="rounded-2xl bg-white p-4 shadow-sm dark:bg-zinc-900">
              <h2 className="mb-3 text-lg font-bold text-makola-green dark:text-green-400">
                Payment method
              </h2>
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                <button
                  ...
                  onClick={() => setPay("momo")}
                  className={cn(
                    "flex items-center justify-center gap-2 rounded-xl border-2 py-4 text-base font-bold",
                    pay === "momo"
                      ? "border-makola-orange bg-makola-orange text-white"
                      : "border-orange-200 dark:border-zinc-700"
                  )}
                >
                  📱 Mobile Money
                </button>
            ...
            {pay === "momo" && (
              <div className="space-y-3 rounded-2xl border-2 border-makola-orange bg-orange-50 p-4 dark:bg-orange-950/30">
                ...
                <p className="text-lg font-bold text-makola-green dark:text-green-400">
                  {momo.momoName}
                </p>
                <p className="text-xl font-extrabold">{momo.momoNumber}</p>
                <p className="text-xs text-gray-500">
                  After sending, enter the Payment ID and Reference from your MoMo SMS / app.
                </p>

                <div>
                  <label className="mb-1 block text-sm font-bold text-makola-green dark:text-green-400">
                ...
                <div>
                  ...
                  <input
                    className="input-lg"
                    placeholder="e.g. Order for Lane 3 / your name"
                    value={momoRef}
            ...
            {error && (
              <div className="rounded-xl bg-red-50 px-4 py-3 text-sm text-red-600 dark:bg-red-950/40">
                {error}
              </div>
            )}
            ...
            <div className="rounded-2xl bg-white p-4 shadow-sm dark:bg-zinc-900">
              ...
              <div className="mt-1 flex justify-between text-base">
                ...
                <span className="font-semibold">{formatGhs(fee)}</span>
              </div>
              <div className="mt-2 flex justify-between border-t border-orange-100 pt-2 text-xl font-bold text-makola-green dark:border-zinc-700 dark:text-green-400">
                <span>Total</span>
                <span>{formatGhs(total)}</span>
              </div>
            </div>

            <button type="button" onClick={placeOrder} className="btn-orange w-full text-xl">
              Place Order
            </button>
            <a
              href={waLink(
                WHATSAPP_NUMBER,
                `I want to order: ${orderText}. Stall: ${stallNote}. Total ~${total} GHS. MoMo ID: ${momoTxnId || "—"} Ref: ${momoRef || "—"}`
              )}
              target="_blank"
              rel="noopener noreferrer"
              className="btn-green w-full text-lg"
            >
